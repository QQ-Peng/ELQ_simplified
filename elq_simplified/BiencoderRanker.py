...
import torch
from mention_detection import mention_detection
...
class BiencoderRanker(nn.Module):
    def __init__(self,params):
        ...
        self.mention_score=mention_detection.MentionScoresHead(bert_output_dim=768, score_method='qa_mlp', max_mention_length=10)
    ...
    def get_embedding_mention(self,bert_output,mention_bounds,method='average_all'):
        ...
        mention_embedding=torch.zeros(mention_bounds2.size(0),mention_bounds2.size(1),bert_output.size(2),dtype=bert_output.dtype,device=bert_output.device)
        for i in range(mention_bounds2.size(0)):
            for j in range(mention_bounds2.size(1)):
    ...
    def forward_ctxt(self,input_ids,segment_type,input_mask,EL=False):
        ...
        mention_score, mention_bounds=self.get_mention_scores(bert_output_ctxt,input_mask.bool())
        if EL:
            mention_embedding=self.get_embedding_mention(bert_output_ctxt,mention_bounds,method='average_all')
        else:
    ...
    def prune_mention(
        ...
        gold_mention_bounds[~(gold_mention_bounds_mask.bool())] = -1  # ensure don't select masked to score
        ...
        for i in range(mention_bounds.size(0)):
            for j in range(mention_bounds.size(1)):
                ...
                for m in range(gold_mention_bounds.size(1)):
                    if mention_bounds[i,j,0]==gold_mention_bounds[i,m,0] and mention_bounds[i,j,1]==gold_mention_bounds[i,m,1]:
...
def main():
    ...
    for epoch in range(num_epoch):
        ...
        for input_ids_ctxt2,input_mask_ctxt2,gold_mention_bounds2,gold_mention_bounds_mask2,gold_entity_local_id2 in dataload:
            #print(n)
            input_ids_ctxt=torch.tensor(input_mask_ctxt2,device='cuda')
            #print(input_ids_ctxt[0,:],type(input_ids_ctxt))
            input_mask_ctxt=torch.tensor(input_mask_ctxt2).to('cuda')
            segment_type_ctxt=torch.zeros_like(input_ids_ctxt).to('cuda')
            gold_mention_bounds=torch.tensor(gold_mention_bounds2,device='cuda')
            gold_mention_bounds_mask=torch.tensor(gold_mention_bounds_mask2).to('cuda')
            gold_entity_local_id=torch.tensor(gold_entity_local_id2,device='cuda')
            optimizer.zero_grad()
            ...
            #mention_embedding: DIM(bs, predmention,output_dim)
            _, mention_scores,mention_bounds,mention_embedding,ment_loss=ranker(
                                                               input_ids_ctxt, segment_type_ctxt, input_mask_ctxt,
            ...
            pres, rec, removeoverlap = compute_precise_recall_overlap(top_k_bounds, torch.tensor(gold_mention_bounds2))

            if el:
                # 加载所有wiki embedding
                all_embedding = torch.load('./Data/wiki_embedding_0.01.t7')
                # 读取index
                indexer = DenseFlatIndexer(1, 5000)
                indexer.deserialize_from('./Data/index_0.01.pkl')
                #print(mention_embedding.shape)
                #DIM: mention_embedding(all_predmention_in_batch, output_dim)
                mention_scores, mention_bounds,mention_embedding,gold_entity_local_id_extend=ranker.prune_mention(
                                                                                             ...
                                                                                             gold_mention_bounds, gold_mention_bounds_mask, gold_entity_local_id
                ...
                biloss=ranker(
                       ...
                       mention_embedding,index_label,el
                       )
            if el:
                biloss.backward()
            else:
