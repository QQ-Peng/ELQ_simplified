...
class MentionScoresHead(nn.Module):
    ...
    def forward(self, bert_output, mask_ctxt):
        ...
        if self.scoring_method[:2] == "qa":
            # (bs, seqlen, 1); (bs, seqlen, 1); (bs, seqlen, 1)             #怎样将不同question的输入tokens调成一样呢？ 在看data_proc时，发现了padding操作
            ...
            # impossible to choose masked tokens as starts/ends of spans     #我自己在写代码时没有考虑到
            ...
            for i in range(mask_ctxt.size(1)):   #用来处理[mask]的分数
...
class BiEncoderModule(torch.nn.Module):
    ...
    def get_ctxt_mention_scores(
        ...
        return self.classification_heads['mention_scores'](  #classification_heads['mention_scores'] 见263行
